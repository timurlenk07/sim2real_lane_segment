...
def main(*, module_type, checkpointPath, showCount, realDataPath, trainDataPath, testDataPath):
    ...
    # Parse model
    if module_type == 'mme':
        model = MMETrainingModule.load_from_checkpoint(checkpoint_path=checkpointPath, num_cls=4)
    elif module_type in ['baseline', 'sandt', 'hm', 'CycleGAN']:
        model = SimpleTrainModule.load_from_checkpoint(checkpoint_path=checkpointPath, num_cls=4)
    else:
    ...
    if testDataPath is not None:
        ...
        for batch in tqdm(testDataLoader):
            ...
            test_conf_matrix += confusion_matrix(label_hat, label, num_classes=4)
            totalWeight += weight
